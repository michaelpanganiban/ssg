	...
	seeMore = () => {
		axios.post('seeMoreLogs', {see:'true'}).then(function(response){
			...
			for(var i = 0; i < response.data.query1.length; i++)
			{
				...
				see_more.push('<li class="time-label"><span class="bg-green">'+response.data.query1[i]['created']+'</span></li>' + children);
			}
			// console.log(see_more.toString());
			return $("#append-logs").append(see_more.toString());
		});
	}
  	render() 
  	{
